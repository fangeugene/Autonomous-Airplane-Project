...
int main(int argc,char* argv[])
	...
	wiiCamera.setTransform(Transform::translate(Vector(0,-30,30)));
  ...
  long useconds;
	...
	while(true)
		...
		vx.normalize();
		std::cout << "wiirot.col(0)\t" << wiirot.getDirection(0) << std::endl;
		std::cout << "wiirot.col(1)\t" << wiirot.getDirection(1) << std::endl;
		std::cout << "wiirot.col(2)\t" << wiirot.getDirection(2) << std::endl;		
		PVector my_vx(wiirot.getDirection(0)[0], wiirot.getDirection(2)[0]);
		PVector my_vy=Geometry::normal(my_vx);
		my_vy.normalize();
		std::cout << "my_vx\t\t" << my_vx << std::endl;
		std::cout << "my_vy\t\t" << my_vy << std::endl;
		std::cout << "vx\t\t" << vx << std::endl;
		std::cout << "vy\t\t" << vy << std::endl << std::endl;
		*/
		...
		if(numValidTargets==4) // Don't use IR tracking if no LEDs are visible Alex: used to be > 0
			{
			/*****************************************************************
      ...
      useconds = end.tv_usec - start.tv_usec;
		...
		sendDeviceState(wiipos.getOrigin()[0],
                    wiipos.getOrigin()[1],
		                ...
		                wiimote->getIRTargetValid(0),
		                wiimote->getIRTargetX(0),
		                wiimote->getIRTargetY(0),
		                wiimote->getIRTargetValid(1),
		                wiimote->getIRTargetX(1),
		                wiimote->getIRTargetY(1),
		                wiimote->getIRTargetValid(2),
		                wiimote->getIRTargetX(2),
		                wiimote->getIRTargetY(2),
		                wiimote->getIRTargetValid(3),
		                wiimote->getIRTargetX(3),
		                wiimote->getIRTargetY(3));
		}
	std::cout<<std::endl;
