...
string buf;
char* outputString = "HEAD%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,TAIL";
...
bool getDeviceState (Vector3d& start_proxy_pos, Matrix3d& start_proxy_rot, vector<Vector2d>& ir_pos) {
  if (receiver.recv(buf)) {
    ...
    ir_pos.clear();
    for (int i=0; i<4; i++)
      if (boost::lexical_cast<double>(vect1[12+i*3].c_str()))
        ir_pos.push_back(Vector2d(boost::lexical_cast<double>(vect1[12+i*3+1].c_str()), boost::lexical_cast<double>(vect1[12+i*3+2].c_str())));
